...
class ApiClient implements Client {
  ...
  ApiClient([HttpClient? inner]) : _inner = inner ?? HttpClient();
  ...
  Future<Response> _sendUnstreamed(
    ...
    if (token != null && token != "") {
      if (headers == null) {
        headers = {Apikeys.authorization: "Bearer ${token.toString()}"};
      } else {
        headers.addAll({Apikeys.authorization: "Bearer ${token.toString()}"});
      }
    }
    ...
    // if (!url.toString().contains(APIServerUrl.lookups) &&
    //     !url.toString().contains(APIServerUrl.getYoutubeVideos) &&
    //     !url.toString().contains(APIServerUrl.getUserBasicDetails)) {
    EasyLoading.show(
        indicator:
    ...
    debugPrint('============ RESPONSE ===============');
    finalResponse.then((value) => developer.log(value.body));
